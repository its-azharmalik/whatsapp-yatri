...
const verifyEmail = async (req, res) => {
	try {
		...
		const url =
			process.env.DOMAIN + '/api/services/verifywithjwt?jwt=' + jwtToken;
		...
		if (response.err != 'NULL') {
			...
			response.acceptedMail.map((mail) =>
				console.log(
					`Mail sent to ${`${mail}`.bold.green} for action "${type_of_action}"`
				)
			);
			res.status(200).json({
	...
	try {
		...
		jwt.verify(jwtToken, process.env.JWT_ACCESS_TOKEN, async (err, data) => {
			if (err) {
				...
				res.status(200).json({
					message:
						'User verified successfully and the details are being updated. Happy surfing !',
					body: user,
				});
			}
		});
...
const resetPasswordRequest = async (req, res) => {
	try {
		...
		const url =
			process.env.DOMAIN + '/api/services/resetpassword?jwt=' + jwtToken;
		const response = await emailService(
			type_of_action,
			url,
			username,
